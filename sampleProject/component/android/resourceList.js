        ...
        RNFS.readDir(RNFS.DocumentDirectoryPath) // On Android, use "RNFS.DocumentDirectoryPath" (MainBundlePath is not defined)
            ...
            .then((statResult) => {
                if (statResult[0].isFile()) {
                    ...
                    console.log("is file "+JSON.stringify(statResult));
    ...
    *getResourceList()
    {
        ...
        let resourceList = yield fetch(Environment.BASE_URL+"/appdemo/getResourceList",{
            method:"POST",
            headers: {
    ...
    constructeDataSource()
    {
        ...
        if(needDownLoad)
        {
            ...
            //ToastLDAndroid.show("finish download",ToastLDAndroid.SHORT);
            this.onClick();
        }
    ...
    *download()
    {
        ...
        let urls = yield fetch(Environment.BASE_URL+"/appdemo/getFilesUrl",{
            method:"POST",
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
        ...
        for(let i=0;i<_targetFiles.length;i++)
        {
            if(_targetFiles[i].exists == 1)
            {
                ...
                downloadTools.startDownload(
                                            ...
                                            this.getSDCardContent.bind(this),//next
        ...
        RNFS.readDir(RNFS.DocumentDirectoryPath) // On Android, use "RNFS.DocumentDirectoryPath" (MainBundlePath is not defined)
            .then((result) => {
                ...
                for(let i = 0;i<result.length;i++)
                {
                    ...
                    console.log(JSON.stringify(result[i]));
                    RNFS.unlink(result[i].path).then(()=>{
                        console.log(result[i] + "has deleted");
                    }).catch((err)=>{
